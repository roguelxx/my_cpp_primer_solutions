...
StrVec::StrVec(std::initializer_list<std::string> li) : StrVec() {
...
std::pair<std::string *, std::string *>
StrVec::alloc_n_move(const std::string *begin, const std::string *end) {
  auto p = alloc.allocate(end - begin);
  auto q = std::uninitialized_copy(std::make_move_iterator(begin), std::make_move_iterator(end), p);
  auto ret_pair = std::make_pair(p, q);
  return ret_pair;
}
...
StrVec::StrVec(StrVec &&ori) {
  auto res_pair = alloc_n_move(ori.begin(), ori.end());
  elements = res_pair.first;
  first_free = cap = res_pair.second;
}
...
StrVec &StrVec::operator=(StrVec &&rhs) {
  if (this != &rhs) {
    auto res_pair = alloc_n_move(rhs.begin(), rhs.end());
    free();
    elements = res_pair.first;
    first_free = cap = res_pair.second;
  }
